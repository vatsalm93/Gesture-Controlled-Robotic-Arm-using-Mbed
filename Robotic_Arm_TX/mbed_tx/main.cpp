...
DigitalOut rst1(PTD4);
DigitalIn grab(PTA12);
...
ADXL345 accelerometer(PTD2,PTD3,PTD1,PTD0);
ITG3200 gyroscope(PTE0, PTE1);
...
int main() {
    ...
    while (1) {
        ...
        if(grab)tosend[2]=99;
        if(!grab)tosend[2]=77;
        tosend[0]=(int)toDegrees(imuFilter.getYaw());
        if(tosend[0]<0)tosend[0]=0;
        tosend[1]=(int)toDegrees(imuFilter.getPitch());
        if(tosend[1]<0)tosend[1]=tosend[1]+180;
        
        ////// convert integers to characters before transmitting   ////////
        
        send[0]=tosend[0]&0xFF;
        send[1]=tosend[1]&0xFF;
        send[2]=tosend[2]&0xFF;
